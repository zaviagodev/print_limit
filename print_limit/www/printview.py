		...
		if frappe.has_permission(doc.doctype, ptype, doc) or frappe.has_website_permission(doc):
			if frappe.db.exists("Print Limit", doc.doctype):
				print_count = frappe.db.count(
					...
					{
						"method": "Print",
						"export_from": doc.doctype,
						"reference_document": doc.name,
						"user": frappe.session.user,
					}
				)
				print_limit = frappe.db.get_value("Print Limit", doc.doctype, "limit")
				if print_limit and print_count >= print_limit:
					raise frappe.PermissionError(_("You have crossed the maximum number of prints allowed for this document."))
			return
